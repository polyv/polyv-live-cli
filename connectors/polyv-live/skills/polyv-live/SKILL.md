...
description_en: Official PolyV Live skill. Manage live channels, stream keys and status, products, coupons, replays, sessions, chat, check-in, Q&A, questionnaires, lotteries, viewers, watch conditions, platform settings and live statistics via the polyv-live-cli. JSON output preferred for automation and data extraction.
allowed-tools: Bash(polyv-live-cli:*)
---

# 保利威云直播 CLI

以已安装的发布版 CLI 为准。不要根据旧示例、缓存文档、历史记忆或其他非发布版资料推断命令语法；先用 `--help` 校验。

## 连接与认证

- 首次连接：执行 `polyv-live-cli auth`，浏览器会打开官方「获取密钥」指引页（https://help.polyv.net/#/live/api/getSecretKey ）；按页面指引取得 appId / appSecret / userId 后，执行 `polyv-live-cli account add workbuddy --app-id <appId> --app-secret <appSecret> --user-id <userId>` 完成配置（用户在对话中直接提供凭证时，由你代为执行此命令）。
- 状态检查：`polyv-live-cli auth status`，输出 `Logged in` 表示凭证可用；`Not logged in` 表示未配置。
- 安全约定：不要在回答、示例或日志中回显用户的 appSecret；签名与凭证管理由 CLI 负责。
- 认证失效（签名错误/凭证缺失）：先运行 `polyv-live-cli auth status` 与 `account current` 诊断，必要时引导用户重新执行 `account add`。
...
CLI 由连接器负责安装（`npm install -g polyv-live-cli`）并托管 Node.js 运行时；执行命令前必须把 `<CLI>` 展开为 `polyv-live-cli`，不要直接执行字面量 `<CLI>`，也不要改用 `npx` 形式。如果 CLI help 与 reference 冲突，必须以 `<CLI> ... --help` 为准。
