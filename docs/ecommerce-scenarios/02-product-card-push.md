...
新品直播首发 —— 在新品直播开播前，为直播间上架「主打款 + 引流款」两件商品（带原价与到手价），并配置两类卡片推送：一张「限时秒杀」手动推送卡片，供主播讲解时一键弹出引导下单；一张「看播满 30 秒领新人红包」观看时长触发卡片，用于留住新进观众、降低跳出。主播讲解商品时弹出商品大卡（product push），形成「上架 → 卡片节奏 → 商品卡推送」的转化闭环。
...
| `product` | `product enabled`、`product add`、`product list`、`product push`、`product cancel-push` | 已执行成功 | 查商品库开关、上架 2 件商品、列表验证、推送商品大卡并取消 |
...
本场景把「上架商品 → 建手动/触发卡片 → 推送商品卡 → 推送/取消卡片」串成一个可照着执行的操作闭环，全部用真实测试频道验证。
...
4. **校验建商品 help**：运行 `product add --help`，确认 `--real-price`、`--price`、`--cover`、`--link-type` 等参数真实存在。
5. **上架主打款**：执行 `product add` 上架旗舰款，原价 399、到手价 299，记录 productId。
6. **上架引流款**：执行 `product add` 上架旅行装洁面小样，原价 39.9、到手价 9.9。
7. **验证商品**：执行 `product list` 确认 2 件商品在架、价格正确。
8. **建手动秒杀卡片**：执行 `card-push create --showCondition PUSH`，记录 cardPushId。
9. **建观看触发红包卡片**：执行 `card-push create --showCondition WATCH --conditionValue 30 --conditionUnit SECONDS`。
10. **卡片列表验证**：执行 `card-push list` 确认 2 张卡片配置正确、`pushStatus=N`。
11. **推送秒杀卡片**：执行 `card-push push`，列表验证 `pushStatus` 由 `N` → `Y`。
12. **取消秒杀卡片**：执行 `card-push cancel`，列表验证 `pushStatus` 由 `Y` → `L`。
13. **商品大卡演练**：执行 `product push --push-card-type bigCard` 推送主打款，再 `product cancel-push` 取消，验证推送可成功发起与回收。
...
| 4 | 22:46 | `product enabled` | `npx --yes polyv-live-cli@rc product enabled -c 7983883 -o json` | 频道 `7983883` | 成功 | `enabled: "N"`（商品库开关默认关闭，但不阻止 add/push） |
...
# 10) 推送 / 取消手动卡片，并用 list 验证 pushStatus 流转
...
2. **【商品库开关不阻断上架】新建频道 `product enabled` 返回 `enabled: "N"`**，但 `product add` 与 `product push` 均成功。即该开关为「商品库展示」相关配置，不阻止通过 API 上架与推送。本场景如实记录初始值 `N`，未尝试打开它（无对应 CLI 开启子命令）。
...
| 商品库开关状态 | 新建频道默认 `N`，不影响上架，但可能影响后台展示 | 如需可在保利威后台开启（CLI 暂无对应开关子命令） |
...
运营在真实直播中复用时，替换占位符即可（`<频道ID>`、`<商品名>`、`<商品链接>`、`<封面图URL>`、`<到手价>`、`<原价>`、`<卡片标题>`、`<倒计时秒>`）：

```bash
# 1) 上架商品（封面图必须是公网可访问 URL！）
npx --yes polyv-live-cli@rc product add \
