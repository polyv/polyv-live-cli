...
- **未覆盖一级命令**：10 / 40（见下表「未覆盖」）

> 进度：30 / 40 = 75%。距离停止条件（40 / 40 全部至少被一个场景真实执行覆盖）仍需继续逐轮补充场景。
