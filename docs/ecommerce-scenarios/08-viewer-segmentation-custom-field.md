...
> 真实执行状态：**已执行成功**（`viewer`、`custom-field` 两族均有业务命令在真实账号 + 真实测试频道/真实测试观众上真实执行成功；其中 `viewer label channel-ref add` 与 `custom-field value save` 真实执行返回成功但当前 CLI 无只读读回路径可验证持久化，作为问题发现记录）
...
电商直播间每场都会沉淀几十到上千条观众记录，但绝大多数团队的观众数据停留在「一堆手机号」的原始状态，无法回答「谁是高意向、谁看过 3 场以上、谁的客单价高」。本场景用 `polyv-live-cli-rc` 的 `viewer` 与 `custom-field` 两族命令，串成一个可照着执行的画像建设闭环：
...
> 1. `viewer label channel-ref add` 对测试频道 `7983932` 返回 `{success:true, data:{channelIds:["7983932"], labelIds:["1282"]}}`，但 `channel get -c 7983932` 的 `labelData` 字段复查仍为空 `[]`，**配置侧（channel get）无法验证 channel-ref 是否持久化**。
> 2. `custom-field value save` 返回 `{success:true}`，但 `custom-field value` 子命令族下**只有 `save`、没有 list/get 读回命令**，且 `viewer get` 不回显观众的自定义字段值，**当前 CLI 无只读路径可读回自定义字段值**。
>
> 两个问题均不影响本场景命令覆盖结论：`viewer` 族的 `list`/`get`/`tag create`/`tag list`/`tag add`（tag add 经 `viewer get` 的 `labels` 数组交叉验证已确认持久化）/`label create`/`label list`/`lottery-wins` 与 `custom-field` 族的 `add`（经 `custom-field list` 交叉验证已确认持久化）/`list` 均真实执行成功，两个命令族均已真实执行覆盖。详见第 12 节问题记录。
...
| `viewer` | `viewer label create` | 已执行失败 → 已执行成功 | 首次「标签名称最大长度为 8」失败；改 6 字「GNHF分层」成功（id `9zu68aethm9eivf1`），`label list` 复查 10→11 |
| `viewer` | `viewer label list` | 已执行成功 | 账号级标签列表（基线 10、新建后 11） |
| `viewer` | `viewer label channel-ref add`（`-c`/`-l`） | 已执行成功（持久化未验证） | 标签关联频道 `7983932` 返回 success，但 `channel get` 的 `labelData` 复查仍空。见第 12.1 节 |
| `viewer` | `viewer lottery-wins` | 已执行成功 | 观众中奖记录查询（totalItems=0，结构完整） |
| `custom-field` | `custom-field add` | 已执行失败 → 已执行成功 | 首次缺 `--custom-field-id` 报 required；补 `gnhf_cust_level` 后成功，`custom-field list` 复查已入库 |
...
> 说明：本场景所有「已执行成功」命令均使用真实账号（`nicksu`）与真实测试频道（`7983932`）/真实测试观众（`4_aksysv...358`）真实执行过，下文「命令执行台账」逐条记录。仅做 `--help` 校验、未真实执行的命令不计入覆盖。`viewer label channel-ref add` 与 `custom-field value save` 真实执行返回 success 但当前无 CLI 只读读回路径可交叉验证，按「已执行成功（持久化未验证）」并作为问题发现记录。`viewer`/`custom-field` 两族均因各自多条业务命令真实执行成功（且 tag add、custom-field add 经交叉验证确认持久化）而计入「已覆盖」。
...
> 创建命令：`npx --yes polyv-live-cli@rc channel create -n "GNHF-电商场景-08-viewer-custom-field-202606230915" -d "..." --scene alone --template alone -o json`，返回 `channelId=7983932`。`channel get` 复核 name 一致、scene/template=alone、pushUrl/pushSecret 已脱敏、`labelData` 在 channel-ref 前后均为 `[]`。
>
...
- 已配置 `polyv-live-cli-rc` 默认账号（本场景用 `nicksu`，production 环境），`account current` / `account list` 可正常返回。
...
| 标签关联频道 | `viewer` | `viewer label channel-ref add -c <channelIds> -l <labelIds>` |
...
5. **建账号级标签并关联频道**：`viewer label create --label-name "GNHF分层"`（注意 ≤8 字）→ `viewer label list` 复查（10→11）→ `viewer label channel-ref add -c 7983932 -l 1282` 把高意向标签关联到测试频道。
...
| 14 | 00:20 | `custom-field add`（首次，缺 id） | `npx ... custom-field add --custom-field-name "GNHF客户等级" --custom-field-type text -f -o json` | — | **失败** | `error: required option '--custom-field-id <id>' not specified`（help 未标 required） |
...
| 17 | 00:20 | `viewer label channel-ref add`（字符串 id 失败） | `npx ... viewer label channel-ref add -c 7983932 -l 9zu68aethm9eivf1 -f -o json` | 7983932 | **失败** | `Error: 标签ID必须是正整数: 9zu68aethm9eivf1`（label 的字符串 id 不被接受） |
| 18 | 00:20 | `viewer label channel-ref add`（数字 id） | `npx ... viewer label channel-ref add -c 7983932 -l 1282 -f -o json` | 7983932 | 成功（持久化未验证） | `{success:true, data:{channelIds:["7983932"], labelIds:["1282"]}}`；但 `channel get` 的 `labelData` 复查仍空（见 12.1） |
...
> 以下为本场景真实执行过的命令（占位符处替换为真实值）。`<CLI>` = `npx --yes polyv-live-cli@rc`。
...
# 标签关联频道（注意：channel-ref 只接受数字 id，即 tag id 1282，不接受 label 的字符串 id）
<CLI> viewer label channel-ref add -c 7983932 -l 1282 -f -o json
```
...
### 12.1 问题：`viewer label channel-ref add` 返回 success，但 `channel get` 的 `labelData` 复查为空

- **现象**：`viewer label channel-ref add -c 7983932 -l 1282 -f -o json` 返回 `{success:true, data:{channelIds:["7983932"], labelIds:["1282"]}}`（输出=请求回显），但随即 `channel get -c 7983932 -o json` 的 `labelData` 字段仍为 `[]`。
- **已做排查**：
  1. 确认 channel-ref 要求 `labelIds` 为**正整数**（首次用 label 的字符串 id `9zu68aethm9eivf1` 直接报 `标签ID必须是正整数`），改用 tag 的数字 id `1282` 后才返回 success——说明 channel-ref 语义上挂的是 **tag（数字 id）** 而非 `viewer label create` 产出的账号级 label（字符串 id）。
  2. 用 `channel get` 的 `labelData` 作为配置侧读回路径，前后均为空。
  3. 怀疑 `labelData` 字段承载的是「频道装修标签」而非「观众分层标签关联」，两者可能是不同存储。
- **结论**：channel-ref 命令本身真实执行成功（返回结构化 success 响应、参数校验通过），但**当前 CLI 配置侧无可靠读回路径**验证其是否持久化。运营若要确认「某标签已关联某频道」，需在保利威后台观众标签页人工复核，或等待后续 CLI 补充只读读回命令。**不能仅凭 channel-ref 返回的 success 就声称「标签已关联频道生效」**。
- **下一步建议**：跟踪 CLI 是否新增 `viewer label channel-ref list`；或向保利威确认 `labelData` 是否为观众标签关联的正确读回字段。
...
- **现象**：
  ...
  - `viewer tag add -l` 与 `viewer label channel-ref add -l` 都要求**数字 id**，即两者挂载的都是 **tag（数字 id）**。
- **结论**：尽管 tag list 的字段名叫 `labelId`，但 `viewer tag`（观众标签，数字 id，可挂观众、可挂频道）与 `viewer label`（账号级标签，字符串 id）是**两套独立的标签实体**，数字 id 与字符串 id 不互通。运营做观众分层打标应优先用 `viewer tag` 体系（数字 id，全链路可挂载、可读回）；`viewer label`（字符串 id）的账号级标签用途与 tag 不同，且其字符串 id 无法被 `tag add` / `channel-ref add` 使用。
- **提示**：本场景为验证「两套实体不互通」这一推断，分别用字符串 id（channel-ref 失败）与数字 id（channel-ref 成功）各执行一次，记录于台账 #17、#18。
...
| 字符串/数字 id 混用报错 | `viewer label channel-ref add` / `viewer tag add` 只接受数字 tag id，传 label 字符串 id 会报 `标签ID必须是正整数` | 关联与打标一律用 `viewer tag create` 产出的数字 id |
| 标签名称长度限制 | `viewer label create --label-name` 最大 8 字（中文按字数计），超长报 `标签名称最大长度为8` | 名称控制在 8 字以内；本场景用 6 字「GNHF分层」 |
| `custom-field add` 的 id 必填 | help 未标 required，但缺 `--custom-field-id` 直接报 required | 新增字段必须同时给 `--custom-field-id`（自定义，建议英文 slug） |
| 成功假象风险 | `channel-ref add`、`custom-field value save` 返回 success 但 CLI 无读回路径 | 不据此声称「已生效」，标注「持久化未验证」，需后台人工复核 |
...
```bash
# 把「高意向」标签关联到主推客单价场次的频道（注意用数字 tag id）
<CLI> viewer label channel-ref add -c <主推频道id> -l <高意向tagId> -f -o json
# 提示：返回 success 但 channel get labelData 不回显，需后台复核
```
...
- **字段值读回能力补齐**：跟踪 CLI 是否新增 `custom-field value list/get` 与 `viewer label channel-ref list`，补齐当前两个「success 无读回」的验证缺口。
