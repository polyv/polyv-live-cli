...
在仓库根目录执行。执行前请先提交或暂存当前未提交改动；命令会检查本地 `gnhf` 和工作区状态，不满足条件会直接退出。
...
cat <<'GNHF_PROMPT' | gnhf \
  --agent claude \
  --worktree \
  --max-iterations 1 \
  --stop-when "docs/ecommerce-scenarios/README.md 显示 polyv-live-cli-rc 的 40 个一级命令全部至少被一个电商营销场景覆盖，并且每个场景文档都包含完整的业务描述、命令链路、验证结果、风险与回滚说明"
